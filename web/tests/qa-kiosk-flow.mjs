...
try{
 ...
 // Daily capability and receipt timers: real Chromium with a controlled clock.
 {
  ...
  kind='duplicate';await choose();await submit();await expect(page.locator('#receipt-screen h1')).toHaveText('Already checked in.');await page.clock.fastForward(3100);await expect(page.locator('#search-screen')).toBeVisible();checks++;
  kind='conflict';await choose();await expect(page.locator('#bible-step')).toBeVisible();const reads=personReads;await submit();await expect(page.locator('#receipt-screen h1')).toHaveText('Already checked in.');ok(personReads===reads+1,'conflict confirmed by exact person read');await page.clock.fastForward(3100);await expect(page.locator('#search-screen')).toBeVisible();
